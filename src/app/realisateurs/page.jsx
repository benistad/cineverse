...
const getDirectorsWithCounts = unstable_cache(
  async () => {
...
export default async function DirectorsPage() {
  const directors = await getDirectorsWithCounts();
  ...
  return (
    <div className="container mx-auto px-6 py-12">
      ...
      <header className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold text-indigo-800 mb-4">Réalisateurs</h1>
        <p className="text-xl text-gray-600">Explorez les filmographies de {directors.length} réalisateurs</p>
      </header>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
        {directors.map(d => (
          <Link key={d.slug} href={`/realisateur/${d.slug}`}
            ...
            <p className="text-sm text-gray-500">{d.count} films</p>
          </Link>
        ))}
      </div>
      ...
      {directors.length === 0 && (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          <p className="text-gray-500">Aucun réalisateur trouvé.</p>
        </div>
      )}
    </div>
  );
